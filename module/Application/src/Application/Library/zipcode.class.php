...
class zipcode_class
{
   public $last_error = "";            // last error message set by this class
   ...
   public function get_zip_details($zip)
   {
      ...
      $sql = "SELECT lat AS lattitude, lon AS longitude,
              FROM postal_codes
              WHERE code='$zip'";
   ...
   public function get_zip_point($zip)
   {
      ...
      $sql = "SELECT lat, lon from postal_codes WHERE code='$zip'";
      $r = mysql_query($sql);
      if (!$r) {
         $this->last_error = mysql_error();
   ...
   public function get_zips_in_range($zip, $range, $sort=1, $include_base)
   {
      ...
      $sql = "SELECT code, lat, lon FROM postal_codes ";
      if (!$include_base) $sql .= "WHERE code <> '$zip' AND ";
      else $sql .= "WHERE ";
